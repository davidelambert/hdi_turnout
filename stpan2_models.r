...
load("stpan2.Rdata")
stpan2$year <- as.numeric(stpan2$year)
...
lm(data = stpan2, tovep ~ hdi)
pool <- plm(data = stpan2, tovep ~ hdi, model = "pooling",
    index = c("state", "year"))
coefficients(pool)
...
lm(data = stpan2, tovep ~ hdi + d14 + d16)
fetime <- plm(data = stpan2, tovep ~ hdi, model = "within",
...
lm(data = stpan2, tovep ~ hdi + factor(st) )
plm(data = stpan2, tovep ~ hdi, model = "within",
    effect = "individual", index = c("state", "year"))
fest <- plm(data = stpan2, tovep ~ hdi, model = "within",
            index = c("state", "year"))
coefficients(fest)
...
# random effects
random <- plm(data = stpan2, tovep ~ hdi, model = "random",
              index = c("state", "year"))
coefficients(random)
...
library(tseries)
stpan2.set <- pdata.frame(stpan2, index = c("state", "year"))
adf.test(stpan2.set$tovep, k = 1)
...
bptest(tovep ~ hdi + factor(st), data = stpan2, studentize = F)
...
extend <- plm(tovep ~ hdi + nonwh_prop, data = stpan2,
