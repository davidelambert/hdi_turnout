...
# variables to pull
stpan2_vars <- c(# Total Pop, Under 18, Non-Citizen 18+, & Median Age,
...
write_csv(stpan2_bu, "stpan2_pull.csv")


# IMPORT IF NOT PULLING ====
stpan2 <- read_csv("stpan2_pull.csv")
stpan2_bu <- stpan2
...
keep <- c("GEOID", "year", "d14", "d16", "state", "poptotal", "noncit",
          "popu18", "popu25", "pop3o", "pop324", "vapacs", "medage", "incmed",
...
stpan2 <- stpan2 %>%
  ...
  mutate(
    d16 = ifelse(stpan2$year == "2016", 1, 0),
    d14 = ifelse(stpan2$year == "2014", 1, 0),
...
# join
stpan2 <- merge(stpan2, states, by = "state")
# rearrange
stpan2 <- stpan2[, c(2:5,1,46,6:45)]
# sort into panels
stpan2 <- arrange(stpan2, GEOID, year)
...
write_csv(stpan2, "stpan2_cleaned.csv")
...
# join
stpan2 <- left_join(stpan2, le, by = c("st" = "st", "year" = "year"))

# Spot Check
View(stpan2[sample(1:153, 10, replace = F), c(1:5, 47)])
...
# get observed Max & Min for each year
  ...
  describe(subset(stpan2, year == "2012", select = lexp))
  # 2012: min = 74.89, max = 81.62
  describe(subset(stpan2, year == "2014", select = lexp))
  # 2012: min = 74.72, max = 81.74
  describe(subset(stpan2, year == "2016", select = lexp))
...
# check out
describe(subset(stpan2, year == "2012", select = health_index))
describe(subset(stpan2, year == "2014", select = health_index))
describe(subset(stpan2, year == "2016", select = health_index))
...
describe(subset(stpan2, year == "2012", select = attain_score))
describe(subset(stpan2, year == "2014", select = attain_score))
describe(subset(stpan2, year == "2016", select = attain_score))
...
# check out
describe(subset(stpan2, year == "2012", select = attain_index))
describe(subset(stpan2, year == "2014", select = attain_index))
describe(subset(stpan2, year == "2016", select = attain_index))
# medians close to 5
...
describe(subset(stpan2, year == "2012", select = enroll_prop))
describe(subset(stpan2, year == "2014", select = enroll_prop))
describe(subset(stpan2, year == "2016", select = enroll_prop))
...
describe(subset(stpan2, year == "2012", select = enroll_index))
describe(subset(stpan2, year == "2014", select = enroll_index))
describe(subset(stpan2, year == "2016", select = enroll_index))
...
describe(subset(stpan2, year == "2012", select = ed_index))
describe(subset(stpan2, year == "2014", select = ed_index))
describe(subset(stpan2, year == "2016", select = ed_index))
...
# Using BLS CPI Inflation calculator, June-June
  ...
  # stpan2[c(sample), "incmed"]
  # # works!
  # rm(test)

# inflation adjustment for real
stpan2$incmed[stpan2$year == "2012"] <- 
  stpan2$incmed[stpan2$year == "2012"] * 1.0503
stpan2$incpc[stpan2$year == "2012"] <- 
  stpan2$incpc[stpan2$year == "2012"] * 1.0503
stpan2$incmed[stpan2$year == "2014"] <- 
  stpan2$incmed[stpan2$year == "2014"] * 1.0112
stpan2$incpc[stpan2$year == "2014"] <- 
  stpan2$incpc[stpan2$year == "2014"] * 1.0112

# check out median income
describe(subset(stpan2, year == "2012", select = incmed))
describe(subset(stpan2, year == "2014", select = incmed))
describe(subset(stpan2, year == "2016", select = incmed))
...
describe(subset(stpan2, year == "2012", select = incpc))
describe(subset(stpan2, year == "2014", select = incpc))
describe(subset(stpan2, year == "2016", select = incpc))
...
describe(subset(stpan2, year == "2012", select = inc_index))
describe(subset(stpan2, year == "2014", select = inc_index))
describe(subset(stpan2, year == "2016", select = inc_index))
...
describe(subset(stpan2, year == "2012", select = hdi))
describe(subset(stpan2, year == "2014", select = hdi))
describe(subset(stpan2, year == "2016", select = hdi))
# median a little above 5, where it should be

# create simulated normals
mean12 <- describe(subset(stpan2, year == "2012", select = hdi))$mean
sd12 <- describe(subset(stpan2, year == "2012", select = hdi))$sd
mean12 <- describe(subset(stpan2, year == "2014", select = hdi))$mean
sd14 <- describe(subset(stpan2, year == "2014", select = hdi))$sd
mean14 <- describe(subset(stpan2, year == "2016", select = hdi))$mean
sd16 <- describe(subset(stpan2, year == "2016", select = hdi))$sd
meanpool <- describe(stpan2$hdi)$mean
sdpool <- describe(stpan2$hdi)$sd
...
stpan2 <- left_join(stpan2, turnout, by = c("st" = "st", "year" = "year"))
...
rm(list=setdiff(ls(), "stpan2"))

write_csv(stpan2, "stpan2.csv")

save(stpan2, file = "stpan2.Rdata")
