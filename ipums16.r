...
# import & back up
ddi <- read_ipums_ddi("sources/ipums/usa_00008.xml")
data16 <- read_ipums_micro(ddi)
data16_bu <- data16
...
# sample -- looks good
View(recode[sample(1:nrow(data16), 12, replace = F), ])
...
save(state, file = "ipums16.Rdata")
write_csv(state, "ipums16.csv")
